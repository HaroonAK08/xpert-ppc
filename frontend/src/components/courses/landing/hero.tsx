...
export function LandingHero() {
  ...
  return (
    <section className="relative overflow-hidden border-b border-border bg-background pb-16 pt-10 sm:pb-20 sm:pt-14">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,_hsla(var(--accent),0.18),_transparent_55%),radial-gradient(ellipse_at_bottom_right,_hsla(var(--primary),0.15),_transparent_50%)]" />
      <LoadFade y={18} duration={0.5} className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          <p className="mb-4 inline-flex items-center rounded-full border border-accent/40 bg-accent/10 px-4 py-1.5 text-[11px] font-bold uppercase tracking-widest text-accent">
            {hero.eyebrow}
          </p>
          ...
          <div className="mb-6 flex flex-wrap items-center justify-center gap-2 text-sm text-muted-foreground">
            ...
            <span className="font-extrabold text-accent">{offer.offerPrice}</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6 md:grid-cols-4">
            {stats.map((s) => (
              <div key={s.label} className="text-center">
                <p className="bg-gradient-to-r from-yellow-300 via-orange-400 to-red-400 bg-clip-text text-3xl font-extrabold text-transparent md:text-4xl">
                  {s.value}
                </p>
